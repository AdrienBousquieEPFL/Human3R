...
class ARCroco3DStereo(CroCoNet):
    ...
    def forward_step(
        ...
        """Run one streaming step of the lighter recurrent model.

        state_args: (state_feat, state_pos, init_state_feat, mem, init_mem) or None on first call.
        trackers:   (last_smpl_tk, last_smpl_id, max_smpl_id, reset_mask) or None on first call.
        Returns (res_cpu, new_state_args, new_trackers). state_args/trackers stay on-device
        and must be threaded back into the next call.
        """
        if trackers is None:
            last_smpl_tk = None
            last_smpl_id = None
            ...
            last_smpl_tk, last_smpl_id, max_smpl_id, reset_mask = trackers

        view = to_gpu(view, device)
        ...
        if selected_imgs_mhmr.size(0) > 0:
            ...
            std = torch.tensor([0.229, 0.224, 0.225], device=device)[None, :, None, None]
            selected_imgs_mhmr = (selected_imgs_mhmr * 0.5 + 0.5 - mean) / std
            mhmr_img_out = [self.backbone(selected_imgs_mhmr)]
        ...
        if self.pose_head_flag:
            ...
            if state_args is None or reset_mask:
        ...
        if last_smpl_tk is not None and smpl_token is not None:
            ...
            # first frame with humans (or first frame after dropout of all tracks)
            if smpl_token is not None:
                smpl_id = torch.arange(n_humans_i, device=device)[None]
        ...
        if use_ttt3r and state_args is not None and not reset_mask:
        ...
        new_state_args = (state_feat, state_pos, init_state_feat, mem, init_mem)
        new_trackers = (last_smpl_tk, last_smpl_id, max_smpl_id, reset_mask)
        return res_cpu, new_state_args, new_trackers
    ...
    def forward_recurrent_lighter(
        ...
        ret_state=False,
        use_ttt3r=False,
        init_state_args=None,
        init_trackers=None,
    ):
        ress = []
        state_args = init_state_args
        trackers = init_trackers
        for view in views:
            res, state_args, trackers = self.forward_step(
                view, device,
        ...
        if ret_state:
            return ress, views, state_args, trackers
        return ress, views
